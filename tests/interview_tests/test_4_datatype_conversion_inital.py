...
def this_is_defininetly_not_a_test():
    ...
    for rec in sample_json_list_of_records:
        ...
        # Do the conversion:
        converted_user_created = convert_ts(
            sample_json_list_of_records["user"]["user_created"]
        )
        converted_date_created = convert_ts(
            sample_json_list_of_records["behaviour"]["date_created"]
        )
    ...
    connection.execute(sql_alchemy_table.insert(), result_list)
